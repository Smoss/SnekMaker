...
def main(unused_argv):
	...
	x = BatchNormalization(momentum=0.9)(x)
	x = Conv2DTranspose(64, kernel_size = 5, strides = (2,2), activation = 'relu')(x)
	x = BatchNormalization(momentum=0.9)(x)
	New_img = Conv2DTranspose(1, kernel_size = (5,5), activation = 'sigmoid')(x)
	...
	for y in range(0, num_samples + num_val_samples):
		if y%10 == 0:
			cv2.imwrite('C:\\Users\\User\\Documents\\SnekMaker\\Sneks\\GraySneks\\Generation_' + str(x) + "_" + str(y) + ".png",sneks[y].astype(int))
		cv2.imwrite('C:\\Users\\User\\Documents\\SnekMaker\\TrainingDataGray\\Squares\\' + str(y) + ".png",sneks[y].astype(int))
	id_model = Sequential()
	...
	id_model.add(Conv2D(64, kernel_size = (5,5), activation = 'relu', input_shape = (img_size, img_size, 1)))
	id_model.add(MaxPooling2D(pool_size = (2,2)))
	#id_model.add(Conv2D(128, kernel_size = (3, 3), activation = 'relu'))
	id_model.add(Conv2D(128, kernel_size = (5,5), activation = 'relu'))
	id_model.add(MaxPooling2D(pool_size = (2,2)))
	...
	id_model_num = 'GAN_20+' + str(img_size)
	...
	for x in range(0,1000):
		train_generator = train_datagen.flow_from_directory(
		        'C:\\Users\\User\\Documents\\SnekMaker\\TrainingDataGray',  # this is the target directory
		        target_size=(img_size, img_size),  # all images will be resized to 128x128
		        batch_size=batch_size,
		        classes = ['Snakes', 'Squares'],
		        class_mode='binary',
		        color_mode='grayscale')  # since we use binary_crossentropy loss, we need binary label
